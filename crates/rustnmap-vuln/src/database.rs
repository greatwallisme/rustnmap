...
use crate::models::{CveEntry, CpeMatch, EpssRecord, KevEntry};
...
impl VulnDatabase {
    ...
    fn init_schema(&self) -> Result<()> {
        ...
        for query in queries {
            self.conn.execute(query, []).map_err(|e| {
                VulnError::database(format!("Failed to execute schema query: {e}"))
            })?;
        }
    ...
    pub fn get_cve(&self, cve_id: &str) -> Result<Option<CveEntry>> {
        ...
        let cve_data: Option<(String, String, Option<f32>, Option<String>, String, Option<String>)> =
            ...
            ).optional().map_err(|e| VulnError::database(format!("Failed to query CVE: {e}")))?;

        let Some((id, description, cvss_v3_base, cvss_v3_vector, published_at_str, modified_at_str)) = cve_data else {
            return Ok(None);
        };
        ...
        let mut ref_stmt = self.conn.prepare(
            "SELECT url FROM cve_references WHERE cve_id = ?1"
        ).map_err(|e| VulnError::database(format!("Failed to prepare reference query: {e}")))?;

        let references: Vec<String> = ref_stmt.query_map(
            params![cve_id],
    ...
    pub fn get_cpe_matches(&self, cve_id: &str) -> Result<Vec<CpeMatch>> {
        ...
        let matches = stmt.query_map(params![cve_id], |row| {
            Ok(CpeMatch {
                cve_id: row.get(0)?,
    ...
    pub fn get_epss(&self, cve_id: &str) -> Result<Option<EpssRecord>> {
        ...
        ).optional().map_err(|e| VulnError::database(format!("Failed to query EPSS: {e}")))?;
    ...
    pub fn get_stats(&self) -> Result<DatabaseStats> {
        ...
        let epss_count: i64 = self.conn.query_row(
