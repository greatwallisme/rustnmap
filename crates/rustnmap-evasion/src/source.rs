...
impl SourceSpoofer {
    ...
    /// # Errors
    ///
    /// Returns an error if the spoofed IP is the same as the real IP,
    /// or if the source port is 0.
...
impl SourceSpooferBuilder {
    ...
    pub fn build(self) -> std::result::Result<SourceSpoofer, crate::Error> {
        ...
        let spoofer =
            SourceSpoofer::new(config, self.real_ip).with_real_port(self.real_port.unwrap_or(0));
...
#[cfg(test)]
mod tests {
    use super::common::{dns_source, doc_source, null_source};
    use super::*;
    ...
    fn test_source_spoofer_spoofed_ip() {
        ...
        assert_eq!(spoofer.source_ip(), "192.0.2.1".parse::<IpAddr>().unwrap());
        assert_eq!(spoofer.real_ip(), real_ip);
        assert!(spoofer.is_ip_spoofed());
    }
    ...
    fn test_source_spoofer_source_addr() {
        ...
        assert_eq!(spoofer.source_addr(), Some("192.0.2.1:80".parse().unwrap()));
    }
    ...
    fn test_common_dns_source() {
        let ip = dns_source();
        assert_eq!(ip, "192.0.2.53".parse::<IpAddr>().unwrap());
    }
